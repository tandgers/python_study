...
for lastFrame in step1.frames:
    ...
    for v in displacementValues_last:
        ...
        DISP.append(v.data)
    np.savetxt(Filename+'DISP.txt',DISP)
...
np.savetxt(Filename+'_after_coordinates.txt',coordinates_springback)
np.save(Filename+'_after_coordinates.npy',coordinates_springback)

b = []
for i in range(len(mianji)-1-500):
    b.append((mianji[i+1] - mianji[0])/mianji[0])
    if b[i]>-0.005:
        tar = i
np.save(Filename+'_mianji.npy',mianji)
