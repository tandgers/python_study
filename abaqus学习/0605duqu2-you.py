...
order_you = [2,1,5,6,7,8,0,10,4,11,12,13,14,15,3,9] #结点顺序
# 计算多边形面积
...
mianji = []
coordinates_bending=[]
for node in part.nodeSets['SUIDAO-YOU'].nodes:
...
zengliangbu = 0
mianji.append(mianji_zuo17(order_you,coordinates_bending))

# 读取回弹变形
step1=odb.steps['Step-2']# 读取odb.steps
for lastFrame in step1.frames:
    


    magnitude = part.nodeSets['SUIDAO-YOU']
    ...
    mianji.append(mianji_zuo17(order_you,coordinates_springback))
    zengliangbu += 1

np.savetxt(Filename+'you_after_coordinates.txt',coordinates_springback)
np.save(Filename+'you_after_coordinates.npy',coordinates_springback)
